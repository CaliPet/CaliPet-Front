...
export function Feed() {
  ...
  const catalogedPets = [
    {
      title: "pages.feeding.categories.item1",
      filter: "dog",
    },
    {
      title: "pages.feeding.categories.item2",
      filter: "cat",
    },
    {
      title: "pages.feeding.categories.item3",
      filter: "bird",
    }
  ]
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [i18n.language]); // Re-fetch data whenever the language changes
  ...
  return (
    <>
      <section className="bg-creme w-full pt-20 flex flex-col items-center">
        ...
        {
          catalogedPets.map(element => (
            <Carousel className="w-full max-w-[1200px] p-5">
              <h3 className="font-bungee">{t(`${element.title}`)}</h3>
              <CarouselContent className="md:-ml-4">
                {
                  foods
                  .filter(food => food.category.includes(element.filter))
                  .map((food) => (
                    <Link to={`/contentFood/${food.id}`} key={food.id}>
